...
module.exports = function(topology, options) {
  ...
  if (options)
    ...
    "do-not-simplify" in options && (doNotSimplify = options["do-not-simplify"]);
  ...
  var features = topology.objects.countries.geometries;
  var doNotSimplifyArcs = [];
  for (var k = 0; k < doNotSimplify.length; k++) {
    var feature = features.filter(function(subDataset) {
      if (subDataset.properties.name === doNotSimplify[k])
        console.log(k);
      return subDataset.properties.name === doNotSimplify[k];
    });

    var flattenedArray = [].concat.apply([], feature[0].arcs);
    flattenedArray = [].concat.apply([], flattenedArray);
    doNotSimplifyArcs = doNotSimplifyArcs.concat(flattenedArray);
  }

  var defaultMinArea = minimumArea;
  topology.arcs.forEach(topology.transform ? function(arc, index) {
    var dx = 0,
    ...
    if (doNotSimplifyArcs.indexOf(index) < 0) {
